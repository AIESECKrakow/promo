...
$ulotk = mysql_query("SELECT * FROM `pp_konkurencje` WHERE type='event' GROUP BY date ORDER BY date ASC")
...
echo "<tr><td class=\"data\">".$data1." <span class=\"dzienk\">".$dzien."</span></td></tr>";
 ...
 $ucze = mysql_query("SELECT * FROM `pp_konkurencje` WHERE date='$data' AND type='event' GROUP BY unibersity")
or die('Strona nie istnieje');
...
$univ = $ucz['unibersity'];
 echo "<tr><td class=\"univ\">".$univ."</td></tr>";
  ...
  $godzin = mysql_query("SELECT * FROM `pp_konkurencje` WHERE date='$data' AND type='event' AND unibersity='$univ'")
or die('Strona nie istnieje');
